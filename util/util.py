...
def log(losses, atob, params, it_val, sampler=None, latent_dim=None, N=4,
    ...
    for i in range(N*N):
        ...
        bp = atob.predict(a)
        img = compose_imgs(a[0], bp[0], is_a_binary=params.is_a_binary,
    ...
    if sampler is not None:
        ...
        a_gen, b_gen = sampler.predict(z_sample, batch_size=1)

        plt.figure(figsize=(10, 6))
        for i in range(N*N):
            img = compose_imgs(a_gen[i], b_gen[i], is_a_binary=params.is_a_binary,
                               is_b_binary=params.is_b_binary)
...
def save_weights(models, log_dir=DEFAULT_LOG_DIR, expt_name=None):
    ...
    if models.atoa is not None:
        models.atoa.save_weights(os.path.join(log_dir, ATOA_WEIGHTS_FILE), overwrite=True)


def load_weights(atoa, atob, d, log_dir=DEFAULT_LOG_DIR, expt_name=None):
    ...
    if atoa:
        atoa.load_weights(os.path.join(log_dir, ATOA_WEIGHTS_FILE))
